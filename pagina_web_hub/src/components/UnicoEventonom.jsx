...
var global=''
export const UnicoEventonom=(props)=>{
...
console.log(global)
    ...
    useEffect(()=>{
        ...
        axios
        .post('/api/CatEventoNom',{"Nombre":props.valor})
        .then((res)=>{
          if(res)
          {
...
<table className="table">
  ...
  {cards.map((x,index)=>
    ...
    <td id={'DD'+x.IdEvento} hidden><textarea name=""   id={'textarea1'+x.IdEvento}  rows="2" >{x.Descripcion.slice(0,150)}</textarea> </td>
     <td id={'AD'+x.IdEvento} >{x.Descripcion.slice(0,150)} </td>
    ...
    <td id={'AS'+x.IdEvento}>{x.estado}</td>
       ...
       <td >  <button type="button" id={'actualizar'+x.IdEvento} hidden  className="btn btn-primary" onClick={(e)=>actualizar(e,x.IdProyecto)}>Actualizar</button></td>
